...
extern int32_t msVar; // м— счетчик
int32_t stopTimer = 0;
...
uint32_t clockFreq; // дл€ определени€ тактовой частоты
...
int main()
{
  ...
  asm("rim"); 
  TIM4_Config(); // на этом таймере таймер в м—
  waitMs(10);
  SPI_Init_RFM70();
  rfm70_init();
  rfm70_channel(0);
  ...
  while(1) {
    if(isTransmit) {
      ...
      if(!GPIO_ReadInputPin(port_SPI_IRQ_CE, pin_IRQ)) {
        ...
        stopTimer = 0; // обнул€ем счетчик остановки
      } else {
        stopTimer++;
        waitMs(1);
        if (stopTimer > 1000) {
